...
stDate CalculateVacationReturnDate(stDate DateFrom, short VacationDays)
{
	...
	for (short i = 1; i <= VacationDays + WeekEndsCounter; i++)
	{
...
int main()
{
	stDate StartVacationDate = ReadDate("\nVacation Starts : \n");
	short VacationDays = ReadPositiveNumber<short>("\nEnter Vacation Days : ");
	cout << "\nReturn Date : " << DateString(CalculateVacationReturnDate(StartVacationDate, VacationDays));
	system("pause>0");
	return 0;
}
