...
{
	/*if the current year is leap, and today's date is 29/2,then after adding a year,the date will become
	28 not 29 of February,because the next year of a leap year is not leap.*/
	if (IsLeapYear(Date.Year) && Date.Month == 2 && Date.Day == NumberOfDaysInMonth(Date.Year, Date.Month))
		--Date.Day;
	++Date.Year;
}

void IncreaseDateByXYears(stDate& Date, short NumberOfYears)
{
...
int main()
{
	...
	cout << "\nadding 10 years is : " << DateString(Date);
	system("pause>0");
	return 0;
}